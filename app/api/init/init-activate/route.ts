...
/**
 * /init phase 7: seed activate singleton (id=1) — platform instance monitoring row.
 */
export async function POST() {
  return withErrorHandling(async () => {
    ...
    const { initActivate } = await import('@/scripts/init-db');
    const stats = await initActivate();

import { Download, Bot, AlertTriangle, Eye, Users, FileText } from 'lucide-react';
...
export default async function AdminIndex() {
  ...
  const [systemStats, downloads, ai, disabledRows, pageViews] = await Promise.all([
    getSystemStats(),
    getDownloadStats(7),
    getAiStats(7),
    getPool().query<any[]>(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        />
        <StatCard label="今日 PV" value={pageViews.todayPv} icon={Eye} href="/admin/analytics" />
        <StatCard label="今日独立访客" value={pageViews.todayUv} icon={Users} href="/admin/analytics" />
        <StatCard
          label="热门页面"
      ...
      <section className="rounded-lg border border-paper-warm bg-paper p-4">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
          ...
          <div>
            <dt className="text-ink-soft text-xs">已禁用用户</dt>
            <dd className="text-xl text-ink font-serif">{disabledUsersCount}</dd>
          </div>
        </dl>
      </section>
    </div>
